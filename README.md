...
Letters live as markdown files in [/newsletters/](/newsletters/), rendered at `/newsletters/<slug>/` and listed at `/newsletters/`. Subscriptions are handled by the same [Cloudflare Worker](/comments-worker/) and D1 database used for comments, with a `subscribers` table holding the email, a random unsubscribe token, timestamps, and the same salted IP hash used elsewhere. Letters are sent via [Resend](https://resend.com/).
